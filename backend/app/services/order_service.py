...
Phase 1: static pharmacy catalogue.
...
def find_pharmacies_with_stock(
    ...
    price = next(
        (v for k, v in _PRICES.items() if k in medicine_lower),
        200,
    )
...
def initiate_mpesa_payment(phone: str, amount_kes: int, order_id: str) -> dict:
    ...
    Returns simulated success until Daraja credentials are configured.
    """
    return {
        "success": True,
        "message": f"M-Pesa payment request sent to {phone}",
        "checkout_request_id": f"ws_CO_{order_id}",
